    ...
    for epoch in range(epochs):
        ...
        if configs.stage==1:  
            torch.save(
                ...
                os.path.join(output_dir, f"{output_prefix}_TD_best.pt"),
            )
        else:
            torch.save(
            brain_encoder.state_dict(),
            os.path.join(output_dir, f"{output_prefix}_BE_best.pt"),
            )
            torch.save(
                text_decoder.state_dict(),
                os.path.join(output_dir, f"{output_prefix}_TD_best.pt"),
            ...
            if configs.B2B_weight!=0: 
                ...
                os.path.join(output_dir, f"{output_prefix}_BD_best.pt"),
                )

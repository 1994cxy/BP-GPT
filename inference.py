...
def run_test(text_encoder, text_decoder, brain_encoder, test_set, configs):
    ...
    for step, data in enumerate(test_set):
        # 仅考虑win_length是hop_length整数倍情况，方便拼出整个story
        assert configs.win_length>=configs.hop_length and configs.win_length%configs.hop_length==0
        if step%(configs.win_length//configs.hop_length)!=0: # 对重叠的窗口，不进行解码
            continue
...
if __name__=='__main__':
    ...
    text_model_path = os.path.join(save_path, f'fmri_prefix_TD_best.pt')
    ...
    brain_model_path = os.path.join(save_path, f'fmri_prefix-BE-best.pt')
